...
int spi_add_device(struct spi_device *spi)
{
	...
	int status;
printk("%s enter spi->chip_select %d ctlr->num_chipselect %d\n",__func__,spi->chip_select,ctlr->num_chipselect);
...
printk("%s read spi_setup\n",__func__);
	...
	if (status < 0)
		...
		printk("registered child %s\n", dev_name(&spi->dev));//dev_dbg(dev, "registered child %s\n", dev_name(&spi->dev));

done:
	mutex_unlock(&spi_add_lock);printk("%s end\n",__func__);
	return status;
}
...
printk("%s ready ctlr->transfer_one_message\n",__func__);
	ret = ctlr->transfer_one_message(ctlr, msg);
printk("%s end ctlr->transfer_one_message ret %d\n",__func__,ret);
	if (ret) {
		dev_err(&ctlr->dev,
...
printk("%s ready of_spi_parse_dt\n",__func__);
...
printk("%s enter\n",__func__);
	...
	for_each_available_child_of_node(ctlr->dev.of_node, nc) {
		if (of_node_test_and_set_flag(nc, OF_POPULATED))
			continue;
printk("%s ready of_register_spi_device\n",__func__);
		spi = of_register_spi_device(ctlr, nc);
		if (IS_ERR(spi)) {printk("%s of_register_spi_device err\n",__func__);
			dev_warn(&ctlr->dev,
...
printk("%s enter\n",__func__);
	...
	if (status)
		return status;
printk("%s check bus_num %d\n",__func__,ctlr->bus_num);
	if (ctlr->bus_num >= 0) {printk("%s bus_num >=0\n",__func__);
		...
		ctlr->bus_num = id;
	} else if (ctlr->dev.of_node) {printk("%s ctlr->dev.of_node\n",__func__);
...
printk("%s get bus_num %d\n",__func__,ctlr->bus_num);
	...
	if (!spi_controller_is_slave(ctlr)) {
		if (ctlr->use_gpio_descriptors) {printk("%s ctlr->use_gpio_descriptors\n",__func__);
		...
		} else {printk("%s ready of_spi_get_gpio_numbers\n",__func__);
...
printk("%s ctlr->num_chipselect %d\n",__func__,ctlr->num_chipselect);
	/*
...
printk("%s ready device_add\n",__func__);
	 ...
	 * memory operations.
	 */
	if (ctlr->transfer) {printk("%s set ctlr->transfer\n",__func__);
		dev_info(dev, "controller is unqueued, this is deprecated\n");
	} else if (ctlr->transfer_one || ctlr->transfer_one_message) {printk("%s set ctlr->transfer_one || ctlr->transfer_one_message\n",__func__);
	...
	mutex_unlock(&board_lock);
printk("%s ready of_register_spi_devices\n",__func__);
...
printk("%s enter\n",__func__);
	...
	if (!ptr)
		return -ENOMEM;
printk("%s ready spi_register_controller\n",__func__);
	ret = spi_register_controller(ctlr);
	if (!ret) {printk("%s ready devres_add\n",__func__);
		*ptr = ctlr;
		devres_add(dev, ptr);
	} else {printk("%s ready devres_free\n",__func__);
		devres_free(ptr);
	}
...
printk("%s ready spi->controller->setup\n",__func__);
	if (spi->controller->setup)
		status = spi->controller->setup(spi);

	if (spi->controller->auto_runtime_pm && spi->controller->set_cs) {printk("%s set controller->auto_runtime_pm\n",__func__);
	...
	} else {printk("%s ready spi_set_cs\n",__func__);
		spi_set_cs(spi, false);
	}
...
printk("%s pass __spi_validate\n",__func__);
	 ...
	 */
	if (ctlr->transfer == spi_queued_transfer) {printk("%s ctlr->transfer == spi_queued_transfer\n",__func__);
		spin_lock_irqsave(&ctlr->bus_lock_spinlock, flags);
		...
		spin_unlock_irqrestore(&ctlr->bus_lock_spinlock, flags);
	} else {printk("%s ctlr->transfer != spi_queued_transfer\n",__func__);
		status = spi_async_locked(spi, message);
	}

	if (status == 0) {printk("%s status == 0\n",__func__);
		/* Push out the messages in the calling context if we
		 * can.
		 */
		if (ctlr->transfer == spi_queued_transfer) {printk("%s status == 0 ctlr->transfer == spi_queued_transfer\n",__func__);
			SPI_STATISTICS_INCREMENT_FIELD(&ctlr->statistics,
						       spi_sync_immediate);
