...
static int spi_ingenic_wait(struct ingenic_spi *ingenic_spi,
			    unsigned long mask, bool condition)
{
...
printk("%s enter\n",__func__);
	...
	if (gpio_is_valid(ingenic_spi->cs_gpios[spi->chip_select])){printk("%s gpio_is_valid\n",__func__);
		gpio_direction_output(ingenic_spi->cs_gpios[spi->chip_select], 0);
}

	if (enable) {printk("%s gpio_is_enable\n",__func__);
		...
		if (gpio_is_valid(ingenic_spi->cs_gpios[spi->chip_select]))
			gpio_direction_output(ingenic_spi->cs_gpios[spi->chip_select], 1);
	} else {printk("%s gpio_is_disable\n",__func__);
		writel(val_ssicr1 | REG_SSICR1_UNFIN,
		       ingenic_spi->base + REG_SSICR1);
...
*spi_ingenic_dma_prepare(struct spi_controller *ctlr, struct spi_device *spi,
			 struct ingenic_spi *ingenic_spi, struct spi_transfer *xfer,
...
static int spi_ingenic_dma_xfer(struct spi_controller *ctlr, struct device *dev,
...
static int spi_ingenic_rx(struct ingenic_spi *ingenic_spi, void *buf,
			  int count, int bits)
{
	int i;
...
static int spi_ingenic_tx(struct ingenic_spi *ingenic_spi,
			 struct spi_transfer *xfer, int count, int bits)
{
	int i, rx_count;
...
static int spi_ingenic_request_dma(struct spi_controller *ctlr, struct device *dev)
{
...
printk("%s enter\n",__func__);
	if (spi->chip_select > 1)
		cs = REG_SSICR1_FRMHL_MASK;
	else
		cs = REG_SSICR1_FRMHL << (spi->chip_select - 1);//这里疑似有问题，当使用cs0时会出现REG_SSICR1_FRMHL << -1的情况;
...
static int spi_ingenic_probe(struct platform_device *pdev)
{
	...
	int cs_gpio, i, num_cs, ret;
	printk("%s enter\n",__func__);
	ctlr = spi_alloc_master(dev, sizeof(struct ingenic_spi));
	if (!ctlr) {
	...
	ingenic_spi->clk = devm_clk_get(dev, "spi");
	if (IS_ERR(ingenic_spi->clk)) {
		dev_err(dev, "Clock not found.\n");
		ret = PTR_ERR(ingenic_spi->clk);
		goto out_free;
	}
	printk("%s check num-cs\n",__func__);
	ret = of_property_read_u32(dev->of_node, "num-cs", &num_cs);
	if (!ret) {
		for (i = 0; i < num_cs; i++) {
			cs_gpio = of_get_named_gpio(dev->of_node, "cs-gpios", i);
			if (cs_gpio == -EPROBE_DEFER)
				break;

			if (gpio_is_valid(cs_gpio)) {
				printk("%s get %d pin %d to %d\n",__func__,i,cs_gpio,num_cs);
				ret = devm_gpio_request(dev, cs_gpio, "ingenic-ssi-cs");
				if (ret) {
					dev_err(dev, "could not request %d gpio\n", cs_gpio);
					ingenic_spi->cs_gpios[i] = -1;
				} else {
					printk("%s set %d pin %d to %d output 1\n",__func__,i,cs_gpio,num_cs);
					gpio_direction_output(cs_gpio, 1);
					ingenic_spi->cs_gpios[i] = cs_gpio;
				}
			} else {
				ingenic_spi->cs_gpios[i] = -1;
			}
	...
	ingenic_spi->base = devm_ioremap_resource(dev, ingenic_spi->mem_res);
	...
	ctlr->mode_bits = SPI_3WIRE | SPI_MODE_3 | SPI_LSB_FIRST | SPI_LOOP | SPI_CS_HIGH
	...
	if (!spi_ingenic_request_dma(ctlr, dev))
		...
		printk("%s DMA not available.\n",__func__);//dev_warn(dev, "DMA not available.\n");
printk("%s devm spi register controller\n",__func__);
	ret = devm_spi_register_controller(dev, ctlr);
printk("%s end devm spi register controller ret %d\n",__func__,ret);
	if (ret) {
