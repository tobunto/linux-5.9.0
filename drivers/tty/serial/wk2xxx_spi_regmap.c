...
#define TIOCGRS485_FLAG	0x542e
#define TIOCSRS485_FLAG	0x542f
...
struct wk2xxx_port 
{
    ...
    uint8_t new_baud0;
    uint8_t new_pres;
    unsigned char dev_id;//yang++
    struct serial_rs485     rs485;

};

/*static struct serial_rs485 wk2xxx_rs485 = {
	.flags= SER_RS485_ENABLED | SER_RS485_RTS_AFTER_SEND //| SER_RS485_RX_DURING_TX,

};*/

static struct wk2xxx_port wk2xxxs[NR_PORTS]; /* the chips */
static struct wk2xxx_port wk2xxxs1[NR_PORTS]; /* the chips */
static struct wk2xxx_port wk2xxxs2[NR_PORTS]; /* the chips */

/*
* This function read wk2xxx of Global register:
*/
static int wk2xxx_read_global_reg(struct wk2xxx_port *s,uint8_t reg,uint8_t *dat)
{
	int ret;
	unsigned int val = 0;

	mutex_lock(&wk2xxxs_reg_lock);
	ret = regmap_read(s->regmap, reg | 0x40, &val);
...
*/
static int wk2xxx_write_global_reg(struct wk2xxx_port *s,uint8_t reg,uint8_t dat)
{
	int ret;

	mutex_lock(&wk2xxxs_reg_lock);
	ret = regmap_write(s->regmap, reg | 0x00, dat);
	mutex_unlock(&wk2xxxs_reg_lock);
...
*/
static int wk2xxx_read_slave_reg(struct wk2xxx_port *s,uint8_t port,uint8_t reg,uint8_t *dat)
{
	int ret;
	unsigned int val = 0;
	 
	mutex_lock(&wk2xxxs_reg_lock);
	ret = regmap_read(s->regmap, 0x40|(((port-1)<<4)|reg), &val);
	mutex_unlock(&wk2xxxs_reg_lock);
...
*/
static int wk2xxx_write_slave_reg(struct wk2xxx_port *s,uint8_t port,uint8_t reg,uint8_t dat)
{
	int ret;

	mutex_lock(&wk2xxxs_reg_lock);
	ret = regmap_write(s->regmap, ((port-1)<<4)|reg, dat);
	mutex_unlock(&wk2xxxs_reg_lock);
...
*/
static int wk2xxx_read_fifo(struct wk2xxx_port *s,uint8_t port,uint8_t fifolen,uint8_t *dat)
{
	int ret;
	uint8_t addr = ((port-1)<<4)|0xc0;
	
	mutex_lock(&wk2xxxs_reg_lock);
	regcache_cache_bypass(s->regmap, true);
	ret = regmap_raw_read(s->regmap, addr, dat, fifolen);
...
*/
static int wk2xxx_write_fifo(struct wk2xxx_port *s,uint8_t port,uint8_t fifolen,uint8_t *dat)
{
	int ret;
	uint8_t addr = ((port-1)<<4)|0x80;//WK2XXX_FDAT|
	
	mutex_lock(&wk2xxxs_reg_lock);
	regcache_cache_bypass(s->regmap, true);
	ret = regmap_raw_write(s->regmap, addr, dat, fifolen);
	regcache_cache_bypass(s->regmap, false);
	mutex_unlock(&wk2xxxs_reg_lock);
...
printk( "--wk2xxx_work---in---\n");
    ...
    do {
             ...
             if(work_start_tx_flag)
             {
              wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_SIER,&rx);
              rx |= WK2XXX_TFTRIG_IEN|WK2XXX_RFTRIG_IEN|WK2XXX_RXOUT_IEN; 
              wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SIER,rx);
             }
...
#endif
    wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE,WK2XXX_PAGE0);//set register in page0
    wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_FSR,dat);
    fsr = dat[0];
    wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_LSR,dat);
    lsr = dat[0];
    wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_SIFR,dat);
    sifr=dat[0];
#ifdef _DEBUG_WK2XXX
...
{  
  ...
  if (fsr& WK2XXX_RDAT)
  {
    wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_RFCNT,dat);
    rfcnt=dat[0];
    if(rfcnt==0)
    {
     rfcnt=255; 
...
#if 1  
    wk2xxx_read_fifo(s,s->port.iobase, rfcnt,rx_dat);
#else
...
else//error
{
    while (fsr& WK2XXX_RDAT)/**/
        {
            wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_FDAT,dat);
        ch = (int)dat[0];
...
printk( "--wk2xxx_tx_chars---in---\n");
       ...
       #endif
            wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_FDAT,s->port.x_char);
            s->port.icount.tx++;
            s->port.x_char = 0;
            goto out;
        }
    ...
    */
    	...
    	wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_TFCNT,dat); 
    	tfcnt= dat[0];
#ifdef _DEBUG_WK2XXX
...
#if 1
	wk2xxx_write_fifo(s,s->port.iobase,i,txbuf);
#else
...
#endif
    	out:wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_FSR,dat);
          fsr = dat[0];
...
static void wk2xxxirq_app(struct uart_port *port)//
{
 ...
 #endif                
    wk2xxx_read_global_reg(s,WK2XXX_GIFR ,dat);
    gifr = dat[0];
#if 0
    ...
    printk(KERN_ALERT "irq_app....gifr:%x  gier:%x  sier1:%x  sier2:%x sier3:%x sier4:%x   sifr1:%x sifr2:%x sifr3:%x sifr4:%x \n",gifr,gier,sier0,sier1,sier2,sier3,sifr0,sifr1,sifr2,sifr3);
        ...
        wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_SIFR,dat);
        sifr = dat[0];
        wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_SIER,dat);
        sier = dat[0];
#ifdef _DEBUG_WK2XXX1
...
static u_int wk2xxx_tx_empty(struct uart_port *port)// or query the tx fifo is not empty?
{
	...
	{
	    wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_FSR,&tx);
...
printk(KERN_ALERT "-wk2xxx_stop_tx------in---\n");
	...
	if(!(s->stop_tx_flag||s->stop_tx_fail))
    {
		...
		if(s->stop_tx_fail)
        {
            ...
            wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SIER,sier);

			
            wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_SIFR,dat);
            sifr=dat[0];
            sifr&= ~WK2XXX_TFTRIG_INT;
            wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SIFR,sifr);
            s->stop_tx_fail =0;
            s->stop_tx_flag=0;
         }
    	else
...
static int wk2xxx_startup(struct uart_port *port)//i
{
        ...
        wk2xxx_read_global_reg(s,WK2XXX_GENA,dat);
        gena=dat[0];
        switch (s->port.iobase)
        {
          case 1:
                  gena|=WK2XXX_UT1EN;
                  wk2xxx_write_global_reg(s,WK2XXX_GENA,gena);
                  break;
          case 2:
                  gena|=WK2XXX_UT2EN;
                  wk2xxx_write_global_reg(s,WK2XXX_GENA,gena);
        ...
        wk2xxx_read_global_reg(s,WK2XXX_GRST,dat);
        grst=dat[0];
        switch (s->port.iobase)
        {
             case 1:
                    grst|=WK2XXX_UT1RST;
                    wk2xxx_write_global_reg(s,WK2XXX_GRST,grst);
                    break;
             case 2:
                    grst|=WK2XXX_UT2RST;
                    wk2xxx_write_global_reg(s,WK2XXX_GRST,grst);
     ...
     printk(KERN_ALERT ":con_wk2xxx_subport bad iobase %d\n", (uint8_t)s->port.iobase);
        ...
        wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_SCR,dat);
        scr = dat[0] | WK2XXX_TXEN|WK2XXX_RXEN;
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SCR,scr);

    //initiate the fifos
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_FCR,0xff);//initiate the fifos
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_FCR,0xfc);
    //set rx/tx interrupt 
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE,1);  
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_RFTL,0x40);
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_TFTL,0X20);
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE,0);  
    //enable the sub port interrupt
        wk2xxx_read_global_reg(s,WK2XXX_GIER,dat);
        gier = dat[0];
        ...
        switch (s->port.iobase)
	{
          case 1:
                gier|=WK2XXX_UT1IE;
                wk2xxx_write_global_reg(s,WK2XXX_GIER,gier);
                break;
          case 2:
                gier|=WK2XXX_UT2IE;
                wk2xxx_write_global_reg(s,WK2XXX_GIER,gier);
      ...
      // request irq
        if(request_irq(s->port.irq, wk2xxx_irq,IRQF_SHARED | IRQF_TRIGGER_LOW, "wk2xxx_irq_gpio", s) < 0)//IRQF_SHARED|
        {
...
#endif
	/*if(s->dev_id == 1){
		wk2xxx_write_slave_reg(s->spi_wk,s->port.iobase,WK2XXX_RS485,0x03);  
		...
		//wk2xxx_write_slave_reg(s->spi_wk,s->port.iobase,WK2XXX_SAEN,0);
...
{   
	...
	wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_SIER ,dat);
	old_sier = dat[0];
	wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SIER ,old_sier&(~(WK2XXX_TFTRIG_IEN | WK2XXX_RFTRIG_IEN | WK2XXX_RXOUT_IEN)));
	...
	do{
        ...
        wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_SCR,dat);
        scr = dat[0];
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SCR ,scr&(~(WK2XXX_RXEN|WK2XXX_TXEN)));
        // set the parity, stop bits and data size //
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_LCR ,lcr);
		/*set cts  and rst*/
		if(fwcr>0){  
#ifdef _DEBUG_WK2XXX
        printk(KERN_ALERT "-conf_wk2xxx_subport-set ctsrts--fwcr=0x%X\n",fwcr);
#endif
				wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_FWCR,fwcr);
				wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE ,1);
				wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_FWTH,0XF0);
				wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_FWTL,0X80);
				wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE ,0);
		}
    
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SIER ,old_sier);
        // set the baud rate //
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE ,1);
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_BAUD0 ,baud0_ss);
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_BAUD1 ,baud1_ss);
        wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_PRES ,pres_ss);
#ifdef _DEBUG_WK2XXX2
         wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_BAUD0,dat);
         printk(KERN_ALERT ":WK2XXX_BAUD0=0x%X\n", dat[0]);
         wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_BAUD1,dat);
         printk(KERN_ALERT ":WK2XXX_BAUD1=0x%X\n", dat[0]);
         wk2xxx_read_slave_reg(s,s->port.iobase,WK2XXX_PRES,dat);
...
static int wk2xxx_config_rs485(struct uart_port *port,
				  struct serial_rs485 *rs485)
{
	struct wk2xxx_port *s = container_of(port,struct wk2xxx_port,port);
	...
	if (rs485->flags & SER_RS485_ENABLED) {
		...
		wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE,0);
		wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_RS485,0x03);  
		wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE,1);
		wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_RRSDLY,0x10);
		wk2xxx_write_slave_reg(s,s->port.iobase,WK2XXX_SPAGE,0);
...
static int wk2xxx_ioctl(struct uart_port *port, unsigned int cmd,
			   unsigned long arg)
{
	...
	switch (cmd) {
	case TIOCSRS485_FLAG:
		
		if (copy_from_user(&rs485, (void __user *)arg, sizeof(rs485))){
			printk("wk2xxx_ioctl copy_from_user(&rs485, (void __user *)arg, sizeof(rs485)) err\n");
			return -EFAULT;
		}
		wk2xxx_config_rs485(port, &rs485);
		return 0;
	case TIOCGRS485_FLAG:
		if (copy_to_user((void __user *)arg, &s->rs485, sizeof(rs485))){
			printk("wk2xxxxx_ioctl copy_to_user((void __user *)arg, \n");
			return -EFAULT;
		}
		return 0;
	default:	
...
static struct uart_driver wk2xxx_uart_driver1 = {
        ...
        dev_name:               "ttysWK1",
#endif
        minor:                  MINOR_START+NR_PORTS,
        nr:                     NR_PORTS,
        cons:                   NULL
};
static struct uart_driver wk2xxx_uart_driver2 = {
...
static int rockchip_spi_parse_dt(struct device *dev)
{
	...
	irq_gpio = of_get_named_gpio_flags(dev->of_node, "irq_gpio", 0,(enum of_gpio_flags *)&irq_flags);
    	...
    	if(!irq)
    	{
		printk(KERN_ERR"wk2xxx gpio_to_request erro irq =%d gpio=%d\n", irq, irq_gpio);
...
static bool wk22xx_regmap_volatile(struct device *dev, unsigned int reg)
{
...
static bool wk22xx_regmap_precious(struct device *dev, unsigned int reg)
{
	switch (reg & 0x0f) {
	case WK2XXX_FDAT:
		return true;
...
static struct regmap_config regcfg = {
	...
	.volatile_reg = wk22xx_regmap_volatile,
	.precious_reg = wk22xx_regmap_precious,
	.max_register = 0xff,
	.read_flag_mask = 0x00,
...
static int wk2xxx_probe(struct spi_device *spi)
{
	...
	ret = regmap_read(regmap,WK2XXX_GENA | 0x40, data);
	if (ret < 0){
		printk(KERN_ERR "regmap_read GENA = 0x%X\n",data[0]);
		return ret;
	}
	ret = regmap_write(regmap,WK2XXX_GENA | 0x00, 0xf5);
	ret = regmap_read(regmap,WK2XXX_GENA | 0x40, data);
	if (ret < 0){
		printk(KERN_ERR "regmap_read GENA1 = 0x%X\n",data[0]);
		return ret;
	}

	ret = regmap_write(regmap,WK2XXX_GENA | 0x00, 0xf0);
	ret = regmap_read(regmap,WK2XXX_GENA | 0x40, data);
	if (ret < 0){
		printk(KERN_ERR "regmap_read GENA2 = 0x%X\n",data[0]);
		return ret;
	}
...
/*	irq = spi->irq;
	sprintf(irqbuf,"wk2xx-irq%d",irq);
	gpio_request(irq, irqbuf);
	gpio_direction_input(irq);
	irq = gpio_to_irq(irq);	*/
	irq = rockchip_spi_parse_dt(&spi->dev);
	if(irq<0)
	{	
		printk( KERN_ERR"config wk2xxx irq erro\n");
		return 1;
	}

    	regmap_read(regmap,WK2XXX_GENA | 0x40, data);
    	if((data[0]&0xf0)!=0x30)
        { 
		printk(KERN_ALERT "read GENA 0x%X != 0x30\n",data[0]);
		...
		if(spi->chip_select == 0){
        		...
        		status = uart_register_driver(&wk2xxx_uart_driver1);
		}
		else if(spi->chip_select == 2){
			
        		status = uart_register_driver(&wk2xxx_uart_driver2);
		}
        	if (status)
        	{
		...
		if(spi->chip_select == 0)
        	    s = &wk2xxxs[i];//container_of(port,struct wk2xxx_port,port);
		else if(spi->chip_select == 1)
        	    s = &wk2xxxs1[i];//container_of(port,struct wk2xxx_port,port);
		else if(spi->chip_select == 2)
        	    s = &wk2xxxs2[i];//container_of(port,struct wk2xxx_port,port);
        	    s->tx_done       =0;
        	    s->spi_wk        = spi;
			    s->port.line     = i;
			...
			if(spi->chip_select == 0){
				...
				s->dev_id=0;
		        	...
		        	status = uart_add_one_port(&wk2xxx_uart_driver1, &s->port);
				s->dev_id=1;
			}
			else if(spi->chip_select == 2){
		        	status = uart_add_one_port(&wk2xxx_uart_driver2, &s->port);
				s->dev_id=2;
			}
       		if(status<0)
          	{
...
static int wk2xxx_remove(struct spi_device *spi)
{
	...
	if(spi->chip_select == 0){
	    	...
	    	for(i =0;i<NR_PORTS;i++)
	       	{
			struct wk2xxx_port *s = &wk2xxxs1[i];
			uart_remove_one_port(&wk2xxx_uart_driver1, &s->port);
	       	}
		uart_unregister_driver(&wk2xxx_uart_driver1);
	}
	else if(spi->chip_select == 2){
	    	for(i =0;i<NR_PORTS;i++)
	       	{
			struct wk2xxx_port *s = &wk2xxxs2[i];
			uart_remove_one_port(&wk2xxx_uart_driver2, &s->port);
	       	}
		uart_unregister_driver(&wk2xxx_uart_driver2);
	}
...
static const struct of_device_id rockchip_spi_wk2xxx_dt_match[] = {
	...
	{ .compatible = "wkmic,wk2xxxspi-2", },
		{ },
};
MODULE_DEVICE_TABLE(of, rockchip_spi_wk2xxx_dt_match);


static const struct spi_device_id wk2xxxspi_id_table[] = {
	{ "wk2xxxspi-0", },
	{ "wk2xxxspi-1", },
	{ "wk2xxxspi-2", },
	{ }
...
MODULE_DEVICE_TABLE(spi, wk2xxxspi_id_table);
