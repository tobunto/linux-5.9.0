...
static int wk2xxx_probe(struct spi_device *spi)
{
	...
	ret = spi_setup(spi);
	if (ret){
		printk(KERN_ERR "spi setup error!!!!\n");
		return ret;
	}*/
//////////////////test spi////////////////////////

	do
	{	printk(KERN_ERR "start wk2xx spi test\n");
...
/*	irq = spi->irq;
	...
	{	
    	...
    	if((dat[0]&0xf0)!=0x30)
        { 
          printk(KERN_ALERT "wk2xxx_probe() spi driver error! GENA = 0x%X\n",dat[0]);
